...
export default function DashboardNotificationsPage() {
    ...
    const filteredNotifications = notifications.filter(notif => {
        ...
        if (activeTab === "System Updates") return type.includes("system") || type.includes("alert");
        return true;
    });
    ...
    const getNotificationBody = (notif: UserNotification) => {
        if (typeof notif.body === 'string') return notif.body;
        if (notif.body && typeof notif.body === 'object' && 'message' in notif.body && typeof (notif.body as any).message === 'string') {
            return (notif.body as any).message;
        }
        return '';
    };
        ...
        <div className="max-w-5xl mx-auto space-y-6">
            ...
            {loading && notifications.length === 0 ? (
                <div className="space-y-4">
                    {[1, 2, 3].map(i => (
                        <Skeleton key={i} className="h-32 w-full rounded-2xl bg-white" />
                    ))}
                </div>
            ) : (
                        ...
                        filteredNotifications.map((notif) => (
                            ...
                            >
                                <div className="space-y-2">
                                    ...
                                    <p className="text-sm text-gray-600 leading-relaxed max-w-2xl pl-12">
                                        {getNotificationBody(notif)} <span className="text-xs text-gray-400 whitespace-nowrap sm:hidden">
                                            {formatTimeAgo(notif.createdAt)}
                                        </span>
                                    </p>
