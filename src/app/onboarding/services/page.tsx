...
export default function ServicesPage() {
    ...
    // The create-service modal renders as a Sheet below `md` and as the
    // original Dialog above it; the field set is shared between both trees.
    ...
    const handleCreateModalClose = () => {
        setOpen(false);
        // Reset form when the modal is dismissed (matches the previous
        // Dialog `onOpenChange(false)` behaviour).
        setServiceName('');
        setServiceDescription('');
        setServicePrice('');
        setHomeServicePrice('');
        ...
        <div className="w-full max-w-[1100px]">
            <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-4 md:p-12 min-h-[600px] flex flex-col">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-10">
                    <div>
                        <h1 className="text-[22px] md:text-3xl font-bold text-gray-900 mb-2">Create Services</h1>
            ...
            <OnboardingCtaBar
                ...
                hint={services.length === 0 ? "Add at least one service to continue" : undefined}
            />

            {/* Add Service modal — bottom Sheet on mobile, the original centered
                Dialog tree verbatim on desktop. The header/footer stay nested
                inside the desktop `p-6` wrapper so desktop padding, gaps and the
                footer's negative-margin bleed render exactly as before. */}
            <ResponsiveModal
                open={open}
                onClose={handleCreateModalClose}
                title="Add New Service"
                footer={isMobile ? <div className="flex flex-col gap-3">{createServiceActions}</div> : undefined}
