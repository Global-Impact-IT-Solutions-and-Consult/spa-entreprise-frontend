...
export function CancelBookingModal({ isOpen, onClose, onConfirm, isLoading }: CancelBookingModalProps) {
    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="bg-white p-0 gap-0 overflow-hidden border-none shadow-2xl rounded-2xl sm:max-w-md">
                ...
                <div className="px-8 pb-8 space-y-6">
                    ...
                    <div className="flex flex-col-reverse sm:flex-row sm:justify-end gap-3 pt-4">
                        <Button
                            variant="outline"
                            onClick={onClose}
                            className="px-8 h-12 w-full sm:w-auto rounded bg-gray-50 border-none text-gray-600 font-bold hover:bg-gray-100"
                        >
                            Cancel
                        </Button>
                        <Button
                            onClick={onConfirm}
                            disabled={isLoading}
                            className="px-8 h-12 w-full sm:w-auto bg-[#E74C3C] hover:bg-[#C0392B] text-white font-bold rounded flex items-center justify-center gap-2"
                        >
